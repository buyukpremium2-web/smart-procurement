...
class Product(Base):
    ...
    last_purchase_price = Column(Numeric(10, 2), nullable=True, default=0)
    selling_price = Column(Numeric(10, 2), nullable=False, default=0)
    expiration_days = Column(Integer, default=7)
    is_active = Column(Boolean, default=True)
    is_market = Column(Boolean, default=True, nullable=False)  # bozor guruhiga tegishlimi (bozorchi shuni ko'radi)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow)
...
class ProductBarcode(Base):
    """Bitta tovarga bir nechta shtrix-kod (1C dan). Skaner bilan qidirish uchun."""
...
class Sale(Base):
    ...
    cost_price = Column(Numeric(10, 2), nullable=True, default=0)  # sotuv paytidagi tannarx (1C cost_price)
    seller_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    sale_date = Column(Date, nullable=False)
    sold_at = Column(DateTime, nullable=True)  # 1C dagi aniq sotuv vaqti (mintaqasiz - 1C bergan vaqt aynan saqlanadi)
    notes = Column(Text)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
    product = relationship("Product")
...
class BuyerLedger(Base):
    """Bozorchi hisob-kitobi (akt-sverka): pul olgan(+), tovar olgan(-)"""
    __tablename__ = "buyer_ledger"
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    buyer_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    entry_date = Column(Date, nullable=False)
    entry_type = Column(String(20), nullable=False)  # 'money' (pul oldi +) yoki 'goods' (tovar oldi -)
    amount = Column(Numeric(12, 2), nullable=False)   # musbat = pul oldi, manfiy = tovar
    comment = Column(Text)
...
class WarehouseJournal(Base):
    """Omborchi jurnali - kontragentlar (korxonalar) reestri"""
    __tablename__ = "warehouse_journal"
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    entry_date = Column(Date, nullable=False)
    amount = Column(Numeric(12, 2), nullable=False, default=0)   # summa
    company_name = Column(String(200), nullable=False)           # korxona nomi (naimenovaniya)
    expeditor = Column(String(200))                              # ekspeditor
    phone = Column(String(50))                                   # telefon nomeri
    comment = Column(Text)                                       # kommentariya
    created_by = Column(UUID(as_uuid=True), ForeignKey("users.id"))
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)


class DeviceApproval(Base):
    """Qurilma tasdiqlash - har kompyuter/telefon admin tomonidan tasdiqlanishi kerak"""
    __tablename__ = "device_approvals"
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    device_id = Column(String(64), unique=True, nullable=False, index=True)  # browser localStorage
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    status = Column(String(20), nullable=False, default="pending")  # pending | approved | rejected
    approval_code = Column(String(8), nullable=True)   # 6-8 belgili kod (admin uchun)
    device_info = Column(String(500))                  # browser/OS ma'lumoti
    ip_address = Column(String(50))                    # IP manzil
    user_agent = Column(String(500))                   # to'liq user-agent
