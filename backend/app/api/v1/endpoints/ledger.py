...
async def get_ledger(
    ...
    """Bozorchi hisobi: barcha yozuvlar + ketma-ket qoldiq"""
    result = await db.execute(
    ...
    for r in rows:
        ...
        entries.append({
            ...
            "type": r.entry_type,
            "money_in": amt if amt > 0 else 0,      # pul oldi
            "goods_out": abs(amt) if amt < 0 else 0, # tovar oldi
            "comment": r.comment,
    ...
    return {
        ...
        "buyer_name": name or "?",
        "balance": round(running, 2),       # musbat = bozorchida pul bor, manfiy = qarz
