...
async def sync_from_1c(
    ...
    # NAMUNA: 1C aynan qanday maydonlar yuborayotganini saqlaymiz (shtrix bormi tekshirish)
    if products and isinstance(products[0], dict):
        log.info("from-1c NAMUNA maydonlar=%s | birinchi_tovar=%s",
...
async def last_sample(key: str = ""):
    """1C oxirgi yuborgan namunani ko'rish (brauzerda). Shtrix bor-yo'qligini tekshirish uchun."""
    if key != SYNC_API_KEY:
        raise HTTPException(status_code=403, detail="key noto'g'ri")
    return {
        "izoh": "1C oxirgi yuborgan ma'lumot namunasi. 'maydonlar' ro'yxatida barcode/shtrix bor-yo'qligini ko'ring.",
