...
# Standart (1-dukon / Buyuk Premium) bozor guruhlari.
_DEFAULT_BOZOR_GURUHLARI = [
...
# Har dukon o'z bozor guruhlarini env orqali berishi mumkin (vergul bilan ajratilgan).
# Masalan 2-dukon (Gagarin): BOZOR_GROUPS="Отдел Овоши,Фрукты,..."
# Env berilmasa -> yuqoridagi standart ro'yxat (1-dukon o'zgarmaydi).
_env_bozor = os.getenv("BOZOR_GROUPS", "").strip()
if _env_bozor:
    _bozor_source = [x for x in _env_bozor.split(",") if x.strip()]
else:
    _bozor_source = _DEFAULT_BOZOR_GURUHLARI
BOZOR_GURUHLARI = {_norm_group(x) for x in _bozor_source}

def _is_bozor(group_name) -> bool:
    return _norm_group(group_name) in BOZOR_GURUHLARI


def _safe_cost(cost, selling=0):
    """1C ba'zan xato tannarx yuboradi (juda katta yoki sotuvdan yuqori).
    Bunday holatda tannarxni 0 deb olamiz - foydaga kirmaydi, lekin INSERT buzilmaydi.
    NUMERIC(10,2) maksimum ~99 999 999.99 (10^8 dan kichik)."""
    try:
        c = float(cost or 0)
    except Exception:
        return 0.0
    if c <= 0:
        return 0.0
    if c >= 100000000:            # ustunga sig'maydi - xato tannarx
        return 0.0
    try:
        s = float(selling or 0)
    except Exception:
        s = 0.0
    if s > 0 and c > s:           # tannarx sotuv narxidan katta - xato
        return round(s, 2)       # tannarxni sotuv narxiga tenglaymiz (foyda 0)
    return round(c, 2)
...
async def sync_from_1c(
    ...
    # ── USTMA-UST from-1c LARNI OLDINI OLAMIZ ──────────────────────────
    # from-1c 34 mingdan ortiq tovarni bitta katta tranzaksiyada yangilaydi va
    # bir necha daqiqa oladi. 1C har 5 daqiqada push qilgani uchun oldingisi
    # tugamasdan yangisi boshlanib, bir xil qatorni qulflab, lock timeout -> 500
    # berardi. Tranzaksiya-darajali advisory lock bilan bir vaqtda faqat BITTA
    # from-1c yozadi. Ikkinchisi "band" deb toza qaytadi (500 emas). Lock
    # commit/rollback da avtomat bo'shaydi - hech qachon osilib qolmaydi.
    # (Kalit 915231 - main.py pull-loop bilan bir xil, ya'ni ular ham o'zaro
    #  bir-birини kutmaydi, bir vaqtda yozmaydi.)
    got_lock = (await db.execute(text("SELECT pg_try_advisory_xact_lock(915231)"))).scalar()
    if not got_lock:
        log.info("from-1c: boshqa sinxron ketyapti, o'tkazib yuborildi")
        return {"message": "Boshqa sinxron ketyapti, o'tkazib yuborildi",
                "skipped": True, "created": 0, "updated": 0}
    ...
    for p in products:
        ...
        try:
            ...
            cost = _safe_cost(_pick(p, "cost_price", "cost", "sebestoimost", "tan_narx", default=0), selling)
            ...
            if existing:
                ...
                if cost > 0:
                    ...
                    existing.last_purchase_price = cost
                if has_amount:
...
async def sales_from_1c(
    ...
    for s in sales:
        ...
        try:
            ...
            cost = _safe_cost(_pick(s, "cost_price", "cost", "sebestoimost", default=0), price)
...
async def _pull_products(db: AsyncSession) -> dict:
    ...
    for it in items:
        ...
        cost = _safe_cost(it.get("cost_price"), selling)
        ...
        if existing:
            ...
            np = Product(
                ...
                minimum_stock=10, current_stock=0, purchase_price=cost,
                last_purchase_price=cost, selling_price=selling, expiration_days=7,
...
async def _pull_sales(db: AsyncSession) -> dict:
    ...
    for s in items:
        ...
        cost = _safe_cost(s.get("cost_price"), price)
        db.add(Sale(
...
# ===== ESKI MA'LUMOTLARNI AVTOMAT TOZALASH (2 oydan oshgani o'chiriladi) =====
@router.get("/cleanup-old")
async def cleanup_old_data(key: str = "", months: int = 2, vacuum: int = 1, db: AsyncSession = Depends(get_db)):
    """2 oydan (months) eski sotuv/harakat/chiqindi/webhook yozuvlarini o'chiradi va joyni bo'shatadi.
    Cron bilan kuniga 1 marta chaqiring. months=2 (default). vacuum=1 -> VACUUM ham qiladi."""
    if key != SYNC_API_KEY:
        raise HTTPException(status_code=403, detail="key noto'g'ri")
    if months < 1:
        months = 1
    cutoff = (datetime.utcnow() + timedelta(hours=5)).date() - timedelta(days=months * 30)

    from sqlalchemy import text as _text
    result = {"cutoff": str(cutoff), "months": months}

    # 1) Eski sotuvlar (sales.sale_date)
    try:
        r = await db.execute(_text("DELETE FROM sales WHERE sale_date < :c"), {"c": cutoff})
        result["sales_ochirildi"] = r.rowcount
    except Exception as e:
        result["sales_xato"] = str(e)

    # 2) Eski ombor harakatlari (stock_movements) - ustun bo'lsa
    for tbl, col in [("stock_movements", "created_at"), ("waste_records", "waste_date"), ("waste_records", "created_at")]:
        try:
            r = await db.execute(_text(f"DELETE FROM {tbl} WHERE {col} < :c"), {"c": cutoff})
            result[f"{tbl}_{col}_ochirildi"] = r.rowcount
        except Exception:
            pass  # jadval/ustun bo'lmasa - o'tkazamiz

    await db.commit()

    # 3) VACUUM (joyni fizik bo'shatish) - alohida, tranzaksiyasiz
    if vacuum:
        try:
            raw = await db.connection()
            await raw.exec_driver_sql("COMMIT")
            for t in ["products", "ai_forecasts", "sales", "product_barcodes", "stock_movements", "waste_records"]:
                try:
                    await raw.exec_driver_sql(f"VACUUM FULL {t}")
                except Exception:
                    try:
                        await raw.exec_driver_sql(f"VACUUM (ANALYZE) {t}")
                    except Exception:
                        pass
            result["vacuum"] = "bajarildi"
        except Exception as e:
            result["vacuum_xato"] = str(e)

    log.info("cleanup-old: %s", result)
    return result
