...
REGOS_OUT_URL = os.getenv(
    "REGOS_OUT_URL",
    "https://integration.regos.uz/gateway/out/22558da4cd4a48caa296fd3640560a61",
)


async def _regos_post(method: str, body: dict):
    url = f"{REGOS_OUT_URL}/v1/{method}"
...
async def _regos_list(method: str, filters=None):
    resp = await _regos_post(method, {"filters": filters or []})
    if not resp.get("ok"):
...
async def napitki_pull(key: str = "", db: AsyncSession = Depends(get_db)):
    ...
    try:
        items = await _regos_list("docsale/get")
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"REGOS docsale xato: {e}")
