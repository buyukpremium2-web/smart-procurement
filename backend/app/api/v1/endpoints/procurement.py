...
async def approve_order(
    order_id: UUID,
    data: Optional[ApproveInput] = None,
    db: AsyncSession = Depends(get_db),
    ...
    # Omborchi miqdorni tahrirlagan bo'lsa - yangilaymiz
        ...
        for upd in data.items:
            ...
            if item:
                ...
                if item.estimated_price:
                    total += upd.bought_qty * float(item.estimated_price)
        if total:
            order.total_estimated_cost = total
...
async def receive_goods(
    ...
    # AVTOMATIK BOZORCHI HISOB-KITOBIGA YOZUV (akt-sverka)
    # Tovar olgani = manfiy (-) summa
    if order.buyer_id and total_actual > 0:
        # Bu zakaz uchun avval yozuv yo'qligini tekshiramiz (qayta qabul qilinmasin)
        existing = await db.execute(
            select(BuyerLedger).where(BuyerLedger.order_id == order.id)
        )
        if not existing.scalar_one_or_none():
            db.add(BuyerLedger(
                ...
                entry_type="goods",
                amount=-float(total_actual),   # manfiy = tovar oldi
                comment=f"Zakaz {order.order_number} (avtomatik)",
                order_id=order.id,
    ...
    await db.commit()
    return {"message": "Tovar qabul qilindi, ombor yangilandi, hisobotga yozildi", "status": order.status.value}
...
async def delete_order(
    ...
    """Noto'g'ri berilgan zakazni o'chirish (faqat admin)."""
    r = await db.execute(select(ProcurementOrder).where(ProcurementOrder.id == order_id))
    order = r.scalar_one_or_none()
    if not order:
        raise HTTPException(status_code=404, detail="Zakaz topilmadi")

    # Qabul qilingan zakazni o'chirib bo'lmaydi (ostatka allaqachon o'zgargan)
    status_val = order.status.value if hasattr(order.status, "value") else str(order.status)
    if status_val in ("received", "completed"):
        raise HTTPException(status_code=400, detail="Qabul qilingan zakazni o'chirib bo'lmaydi")

    order_num = order.order_number
    # Bog'liq buyer_ledger yozuvlarini o'chiramiz (FK to'sig'i bo'lmasin)
    await db.execute(delete(BuyerLedger).where(BuyerLedger.order_id == order_id))
    # ProcurementItem CASCADE bilan avtomatik o'chadi; orderni o'chiramiz
    await db.delete(order)
    await db.commit()
    return {"message": "Zakaz o'chirildi", "order_number": order_num}
