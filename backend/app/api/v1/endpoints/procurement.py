...
from app.models.models import ProcurementOrder, ProcurementItem, ProcurementStatus, Product, User
...
async def get_procurement_order(
    ...
    return {
        ...
        "total_actual_cost": float(order.total_actual_cost or 0),
...
@router.patch("/{order_id}/approve", dependencies=[Depends(require_roles("warehouse_manager", "admin"))])
async def approve_order(
    ...
    if order.status != ProcurementStatus.buyer_confirmed:
        raise HTTPException(status_code=400, detail="Bu zakaz tasdiqlanishi mumkin emas")
    ...
    return {"message": "Zakaz tasdiqlandi va qabul qilishga tayyor", "status": order.status}


@router.patch("/{order_id}/reject", dependencies=[Depends(require_roles("warehouse_manager", "admin"))])
async def reject_order(
    ...
    return {"message": "Zakaz rad etildi", "status": order.status}
