...
async def range_stock_report(
    ...
    if scope == "napitki":
        pq = pq.where(Product.product_code.like("R-%"))     # faqat REGOS (napitki) tovarlari
    elif scope != "all":
        pq = pq.where(Product.is_market == True)              # faqat bozor
    ...
    for p in products:
        ...
        # 'all'/'napitki' rejimida: faqat sotuv yoki chiqindi bo'lgan tovarlar (bo'sh qator chiqmasin)
        if scope in ("all", "napitki") and sold_raw is None and waste_qty == 0:
            continue
...
async def product_daily_detail(
    product_id: str,
    report_date: date = Query(...),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Bitta tovarning bir kundagi HAR BIR sotuvi: vaqt (soat:minut), soni, chek.
    1C bilan solishtirib qaysi sotuv kelmaganini tekshirish uchun."""
    result = await db.execute(
        select(Sale.sold_at, Sale.quantity, Sale.unit_price, Sale.notes)
        .where(Sale.product_id == product_id, Sale.sale_date == report_date)
    ...
    for row in result:
        ...
        total_qty += q
        doc = (row.notes or "").replace("1C #", "").strip()
        sales.append({
            "time": t.strftime("%H:%M") if t else "\u2014",
            "qty": q,
            "value": round(q * float(row.unit_price or 0)),
            "doc": doc or "\u2014",
        })
    return {
