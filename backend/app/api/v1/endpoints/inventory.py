...
async def list_waste(
    ...
    result = await db.execute(
        select(WasteRecord).where(WasteRecord.waste_date == target).order_by(WasteRecord.created_at.desc())
    )
    rows = result.scalars().all()
    out = []
    for w in rows:
        # Mahsulot nomi
        pr = await db.execute(select(Product.name, Product.unit).where(Product.id == w.product_id))
        prod = pr.first()
        # Sotuvchi nomi
        ur = await db.execute(select(User.full_name).where(User.id == w.seller_id))
        seller = ur.scalar_one_or_none()
        out.append({
            "id": str(w.id),
            "product_name": prod[0] if prod else "?",
            "unit": prod[1] if prod else "",
...
async def daily_stock_report(
    ...
    products_r = await db.execute(
        select(Product).where(Product.is_active == True, Product.is_market == True).order_by(Product.name)
    )
...
@router.get("/product/{product_id}/daily")
async def product_daily_report(
    product_id: UUID,
    ...
    """Bitta tovar uchun kunlik sotuv hisoboti (davr bo'yicha)."""
    if (date_to - date_from).days > 366:
        raise HTTPException(status_code=400, detail="Maksimal 1 yil")
    ...
    # Kunlik sotuv (sana bo'yicha guruhlangan)
    rows = await db.execute(
        select(
            Sale.sale_date,
            func.sum(Sale.quantity).label("qty"),
            func.sum(Sale.quantity * Sale.unit_price).label("value")
        )
    ...
    days = [
        {"date": str(r.sale_date), "qty": float(r.qty or 0), "value": round(float(r.value or 0))}
        for r in rows
    ]
    total_qty = sum(d["qty"] for d in days)
    total_value = sum(d["value"] for d in days)

    # Chiqindi (davr bo'yicha)
    wr = await db.execute(
        select(func.sum(WasteRecord.quantity))
        .where(WasteRecord.product_id == product_id)
        .where(WasteRecord.waste_date.between(date_from, date_to))
    )
    total_waste = float(wr.scalar() or 0)
...
async def range_stock_report(
    date_from: date = Query(...),
    date_to: date = Query(...),
    scope: str = Query("bozor", description="'bozor' = faqat bozor tovarlari, 'all' = hamma (obshi ham)"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    ...
    pq = select(Product).where(Product.is_active == True)
    if scope != "all":
        pq = pq.where(Product.is_market == True)
    pq = pq.order_by(Product.name)
    products_r = await db.execute(pq)
    products = products_r.scalars().all()
    ...
    for p in products:
        pid = str(p.id)
        sold_raw = sold_map.get(pid)
        waste_qty = waste_map.get(pid, 0)
        # 'all' rejimida: faqat sotuv yoki chiqindi bo'lgan tovarlar (34k qator qaytmasin)
        if scope == "all" and sold_raw is None and waste_qty == 0:
            continue
        sold = sold_raw or {"qty": 0, "value": 0}
        items.append({
            "product_id": pid,
            "product_code": p.product_code or "",
            "product_name": p.name,
...
async def sales_summary_report(
    ...
    """Barcha tovarlar bo'yicha kunlik JAMI sotuv (summar — diagramma uchun).
    Bozor va menejer (obshi) tovarlar birga hisoblanadi. Tovar bo'yicha emas, kun bo'yicha."""
    if (date_to - date_from).days > 92:
        raise HTTPException(status_code=400, detail="Maksimal 3 oy")
    ...
    days = [
        {"date": str(r.sale_date), "value": round(float(r.value or 0)), "qty": float(r.qty or 0)}
        for r in rows
    ]
    return {
        "date_from": str(date_from),
...
@router.get("/product/{product_id}/hourly")
async def product_hourly_sales(
    product_id: str,
    report_date: date = Query(...),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """Bitta tovarning bir kundagi sotuvi — SOAT bo'yicha (vaqti bor yozuvlar).
    Qaysi soatda qancha sotilganini ko'rsatadi (1C bilan solishtirish uchun)."""
    rows = await db.execute(
        select(
            func.extract('hour', Sale.sold_at).label('hour'),
        ...
        .group_by(func.extract('hour', Sale.sold_at))
        .order_by(func.extract('hour', Sale.sold_at))
    )
    hours = [
        {
            "hour": int(r.hour),
    ...
    # Vaqtsiz (eski) yozuvlar — sold_at yo'q
    nv_r = await db.execute(
        select(func.count(), func.sum(Sale.quantity)).where(
            Sale.product_id == product_id,
            Sale.sale_date == report_date,
            Sale.sold_at.is_(None),
        )
    )
    nv = nv_r.first()
    no_time_count = int(nv[0] or 0)
    no_time_qty = float(nv[1] or 0)
    return {
        "product_id": str(product_id),
        "date": str(report_date),
        "hours": hours,
        "no_time_count": no_time_count,
        "no_time_qty": no_time_qty,
        "total_qty": sum(h["qty"] for h in hours) + no_time_qty,
    }
