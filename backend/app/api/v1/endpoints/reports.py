...
async def store_info():
    """Dukon nomi (brend). Har filial STORE_NAME env orqali beradi.
    1-dukon: PREMIUM (default), 2-dukon: GAGARIN. Auth talab qilmaydi -
    login sahifasida ham ko'rinadi."""
    return {"name": os.getenv("STORE_NAME", "PREMIUM")}
...
async def sales_summary(
    ...
    profit_expr = Sale.quantity * (Sale.unit_price - Sale.cost_price)
    prof_q = _scoped(
        select(
        ...
        .select_from(Sale).join(Product, Product.id == Sale.product_id)
        .where(and_(*filters, Sale.cost_price > 0))
    )
