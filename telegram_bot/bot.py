...
async def cmd_tasdiqla(message: Message, state: FSMContext):
    """Admin: /tasdiqla KOD - qurilmani tasdiqlash"""
    tg_id = message.from_user.id
    token = get_token(tg_id)
    if not token:
        await message.answer("⛔️ Avval login qiling: /start")
        return
    ...
    status, result = await api_request(
        "POST", "devices/approve-by-code",
        json_data={"code": code}, token=token
    )
    if status == 200 and result:
        await message.answer(
            f"✅ *Qurilma tasdiqlandi!*\n\n"
            f"👤 {result.get('user_name', '')}\n"
            f"📱 {result.get('device_info', 'noma_lum')[:100]}",
            parse_mode="Markdown"
        )
    elif status == 404:
        await message.answer("❌ Kod topilmadi yoki allaqachon ishlatilgan.")
    elif status == 403:
        await message.answer("⛔️ Faqat admin tasdiqlay oladi.")
    else:
        await message.answer(f"❌ Xato: {result}")


@router.message(Command("rad"))
async def cmd_rad(message: Message, state: FSMContext):
    """Admin: /rad KOD - qurilmani rad etish"""
    tg_id = message.from_user.id
    token = get_token(tg_id)
    if not token:
        await message.answer("⛔️ Avval login qiling.")
        return
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) < 2:
        await message.answer("📝 `/rad KOD`", parse_mode="Markdown")
        return
    code = parts[1].strip().upper()
    status, result = await api_request(
        "POST", "devices/reject-by-code",
        json_data={"code": code}, token=token
    )
    if status == 200:
        await message.answer("❌ Qurilma rad etildi")
    else:
        await message.answer(f"❌ Xato: {result}")
