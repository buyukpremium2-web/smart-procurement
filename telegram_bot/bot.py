...
def main_menu(role: str) -> ReplyKeyboardMarkup:
    menus = {
        "seller": [
            ["📦 Sotuv kiritish", "📋 Qo'shimcha zakaz"],
            ["📊 Bugungi sotuvlar", "🏪 Ombor holati"],
        ],
        "buyer": [
            ["🤖 AI tavsiyalar", "📝 Zakaz yaratish"],
            ["📊 Zakazlar ro'yxati", "💰 Narxlar"],
        ],
        "warehouse_manager": [
            ["✅ Tasdiqlash kutmoqda", "📊 Ombor holati"],
            ["📋 Barcha zakazlar"],
        ],
        "goods_receiver": [
            ["📥 Tovar qabul qilish", "📋 Qabul tarixi"],
        ],
        "admin": [
    ...
    buttons = menus.get(role, menus["seller"])
    buttons.append(["🚪 Chiqish"])
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Web sayt tugmasi (har doim ko'rsatamiz)
    web_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌐 Web saytni ochish", url=BACKEND_URL)]
    ])
    ...
    await message.answer(
        "🌿 *Buyuk Premium*\n\n"
        "Meva-Sabzavot Do'koni uchun ERP Tizimi\n\n"
        "🔐 Tizimga kirish uchun loginingizni yuboring:",
        parse_mode="Markdown",
        reply_markup=ReplyKeyboardRemove()
    )
    await message.answer(
        "🌐 Yoki web saytdan foydalaning:",
        reply_markup=web_kb
    )
