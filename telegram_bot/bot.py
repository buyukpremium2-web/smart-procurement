...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # 1. Deep-link token bormi? (/start abc123xyz)
    parts = (message.text or "").split(maxsplit=1)
    invite_token = parts[1].strip() if len(parts) > 1 else None

    if invite_token:
        # Token orqali bog'lanish
        status, result = await api_request(
            "POST", "users/connect-telegram",
            json_data={"invite_token": invite_token, "telegram_id": tg_id}
        )
        if status == 200 and result:
            role_label = ROLE_LABELS.get(result.get("role", ""), result.get("role", ""))
            await message.answer(
                f"✅ *Muvaffaqiyatli ulandingiz!*\n\n"
                f"Salom, *{result['full_name']}* 👋\n"
                f"Rol: {role_label}\n\n"
                f"🔔 Endi bildirishnomalar shu yerga keladi.\n"
                f"👇 Pastdagi tugma orqali tizimga kiring:",
                parse_mode="Markdown",
                reply_markup=main_menu(result.get("role", ""))
            )
        else:
            await message.answer(
                "❌ *Havola yaroqsiz yoki allaqachon ishlatilgan.*\n\n"
                "Yangi havola uchun admin bilan bog'laning.",
                parse_mode="Markdown"
            )
        return

    # 2. Token yo'q - bu foydalanuvchi ro'yxatda bormi tekshiramiz
    status, result = await api_request("GET", f"users/check-telegram/{tg_id}")
    if status == 200 and result and result.get("allowed"):
        role_label = ROLE_LABELS.get(result.get("role", ""), result.get("role", ""))
        extra = ""
        if result.get("role") == "admin":
            extra = "\n\n👑 *Admin buyruqlari:*\n`/havola` — foydalanuvchilar ro'yxati\n`/havola @username` — havola yaratish"
        await message.answer(
            f"👋 Xush kelibsiz, *{result['full_name']}*!\n"
            f"Rol: {role_label}\n\n"
            f"🔔 Bildirishnomalar yoqilgan.\n"
            f"👇 Pastdagi tugma orqali tizimga kiring:{extra}",
            parse_mode="Markdown",
            reply_markup=main_menu(result.get("role", ""))
        )
        return

    # 3. Begona - kirish taqiqlangan
    await message.answer(
        "🌿 *Buyuk Premium*\n\n"
        "⛔️ *Kirish faqat taklif havolasi orqali.*\n\n"
        "Tizimga ulanish uchun administratordan maxsus havola so'rang.\n"
        "Havola ko'rinishi: `t.me/bot?start=...`",
        parse_mode="Markdown"
    )
